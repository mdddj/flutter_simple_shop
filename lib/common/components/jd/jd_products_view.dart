  ...
  Widget renderListWidget(){
    return Consumer(builder: (BuildContext context, T Function<T>(ProviderBase<Object?, T>) watch, Widget? child) {
      ...
      return WaterfallFlow.count(
        ...
        mainAxisSpacing: 12,
        crossAxisSpacing: 12,
        crossAxisCount: 2, children: products.map(renderItem).toList(),);
    },);
  }
                  ...
                  children: [
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8,vertical: 2),
                        ...
                        child: const Text('京东',style: TextStyle(color: Colors.white),)),
                    Text('销量${item.inOrderCount30Days}',style: const TextStyle(
                      color: Colors.grey
                    ),)
                  ],
                ),
                Text(item.skuName,maxLines: 1,overflow: TextOverflow.ellipsis,style:
                  const TextStyle(fontSize: 12),).marginOnly(top: 12),
