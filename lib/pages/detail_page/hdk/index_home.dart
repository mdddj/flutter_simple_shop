...
class HaoDanKuDetailItemState extends ConsumerState<HaoDanKuDetailItem>
          ...
          children: <Widget>[
            ExtendedNestedScrollView(
              ...
              headerSliverBuilder: (
                ...
                return <Widget>[
                  SliverToBoxAdapter(child: buildGoodsSwiper()),
  ...
  Widget _writeButton() {
    return ImageWrapper(
      child: IconButton(
        ...
        onPressed: () {
          context.push(
            pagerUtil.resourceWrite.routername,
      ...
      children: <Widget>[
        ...
        OutlinedButton(
          onPressed: () async {
            if (couponLinkResult != null) {
              utils.copy(
                couponLinkResult!.longTpwd ?? '无优惠券',
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: <Widget>[
          ...
          DetailImagesWidget(images: info!.detailPics, hideTitle: true),
        ],
      ),
    );
  }
  ...
  Widget buildSliverToBoxAdapterShop({bool isSliver = true}) {
    ...
    return SliverToBoxAdapter(child: _shopInfo != null ? widget : Container());
  }

  // 占位
  Widget buildSliverToBoxAdapterPlaceholder({bool isSliver = true}) {
    Widget widget = Container(height: 12);
    if (!isSliver) {
      return widget;
    }
    return SliverToBoxAdapter(child: Container(height: 12));
  }
        ...
        child: Container(
          ...
          child: Row(
            children: <Widget>[
              ...
              Container(
                alignment: Alignment.center,
                child: Text('立即领券 >', style: context.textTheme.titleSmall),
              ),
            ],
          ),
        ),
    ...
    return AppBar(
      ...
      title: Row(
        ...
        children: <Widget>[
          Expanded(
            child: TabBar(
              tabs: const [Tab(text: '宝贝'), Tab(text: '详情'), Tab(text: '推荐')],
  ...
  Widget buildContainer({Widget? swiper, double? width}) {
    return SizedBox(height: width ?? context.screenWidth, child: swiper);
  }
  ...
  Future<String> initDatas() async {
    ...
    try {
      ...
      final result = await MyNewApiDetailInfo().request(
        RequestParams(
...
class Test extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Image.asset('assets/icons/fanli.png');
  }
}
