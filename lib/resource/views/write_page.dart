...
class _MyResourceWritePageState extends State<MyResourceWritePage> {
  ...
  ///分享的产品
  late ProductShare? share = params.mapOrNull((value) => null, dynWritePageParam: (value) => value.productShare);

  late String appbarTitle = params.mapOrNull((value) => "发布", dynWritePageParam: (value) => value.title) ?? "发布";
        ...
        actions: [
          IconButton(
              onPressed: () {
                ...
                context.askOk(const AskOkDialogParams(title: Text("提示"), content: Text('请文明发言,不要发布违规内容'), cancelText: '关闭', okText: '我知道了'));
              },
              icon: const Icon(
                Icons.help,
                color: Colors.grey,
              )),
          FilledButton(onPressed: content.isEmpty ? null : _submit, child: const Text("发布")),
      ...
      body: BodyExpandedWidget(
        ...
        child: Column(
          children: [
                    ...
                    PictureSelection(
                      ...
                      customRender: (images, controller, showAddButton) {
                        return Container(
                          ...
                          decoration: BoxDecoration(border: Border.all(color: context.colorScheme.surfaceTint, width: .2), borderRadius: BorderRadius.circular(12)),
                            ...
                            child: Row(
                              ...
                              children: [
                                ...images.map((e) => ConstrainedBox(
                                      ...
                                      child: _ImageShow(image: e, onDelete: controller.remove),
                                    )),
                                if (showAddButton)
                                  Padding(
                                    padding: const EdgeInsets.all(8.0),
                                    child: InkWell(onTap: controller.showMenu, child: const ImageWrapper(child: Icon(Icons.add))),
                                  )
                              ],
                            ),
                          ),
            ...
            if (!_showImageUploadComp && showProductShareCard) _ShareProductWidget(model: share)
          ],
        ),
      ),
    );
...
class _ImageShow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final param = ImageParams(size: double.infinity, borderRadius: BorderRadius.circular(12), shape: BoxShape.rectangle, fit: BoxFit.cover);
    return Container(
      ...
      child: Stack(
        children: [
          image.when(
            file: (file) {
              return ImageView(image: MyImage.filePath(filePath: file.path, params: param));
            },
            network: (url) {
              return ImageView(
...
class SelectResCategory extends StatefulWidget {
  ...
  const SelectResCategory({super.key, required this.initCategoryName, this.onChange, required this.selectOther});
...
class _SelectResCategoryState extends State<SelectResCategory> {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: SingleChildScrollView(
          ...
          child: Row(
            children: [
              Chip(label: Text(selectResCategory?.name ?? widget.initCategoryName)),
              //后面放一些推荐的
              if (widget.selectOther) TextButton(onPressed: _selectMore, child: const Text('选择其他')).marginOnly(left: 12)
            ],
          )),
    );
  }
  ...
  ///选择其他群组
  void _selectMore() {
    context.navToWidget<ResourceCategory>(to: const SelectResourceCategoryPage()).then((value) {
      if (value != null) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      child: Padding(
        ...
        child: Row(
          children: [
            ImageView(image: MyImage.network(url: model!.image, params: ImageParams(size: size, borderRadius: BorderRadius.circular(12), shape: BoxShape.rectangle, fit: BoxFit.cover))),
            const SizedBox(
              width: 12,
            ),
            Expanded(
